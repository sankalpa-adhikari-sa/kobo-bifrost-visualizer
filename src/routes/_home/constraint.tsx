...
const QuestionCard = ({ question, surveyMap }) => {
  const constraintVariables = extractVariables(question.constraint);
  const labelVariables = extractVariables(question.label || "");
  const constraintMessageVariables = extractVariables(
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-2">
        ...
        {question.constraint && (
          <div>
            <strong className="font-medium">Constraint:</strong>{" "}
...
function ConstraintPage() {
  ...
  const questionTypes = Array.from(
    new Set(constraintRows.map((row) => getBaseType(row.type))),
  ).sort();

  const filteredRows = constraintRows.filter((row) => {
    const matchesSearch =
      searchTerm === "" ||
  ...
  return (
    <div className="p-4">
      <h1 className="text-2xl font-bold mb-6">Constrained Survey Questions</h1>
